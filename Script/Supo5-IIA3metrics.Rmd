...
\begin{tcolorbox}[breakable, title=Simultaneity Bias in OLS\footnote{Wooldridge (2021, $7^{th}$ ed) Section 16-2: Simultaneity Bias in OLS}, skin=enhancedlast]
...
\begin{itemize}
\item \textit{Instrument relevance:} whereby the tax rate must have non-trivial explanatory power for police per capita, i.e. 
$Cov(taxpc, polpc)\neq 0$;
...
\begin{align}
{hours}_i &= \beta_0 + \beta_1\ {wage}_i + \beta_2\ {kids}_i + u_i \label{eq:FQB1} \\ 
{wage}_i &= \gamma_0 + \gamma_1\ {hours}_i + \gamma_2\ {kids}_i + v_i. \label{eq:FQB2}
\end{align}
...
\begin{description}
\item[Answer:]
Equation (\ref{eq:FQB1}) is the labor supply decision by workers;

Equation (\ref{eq:FQB2}) is the labor demand decision of firms, who typically pay higher wages for those who work full time.
...
\begin{description}
\item[Answer:]
Not consistent due to simultaneity.
\end{description}
