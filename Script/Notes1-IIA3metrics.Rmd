...
= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} = \frac{\displaystyle\sum_{i=1}^nX_i(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} = \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})Y_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}.
\]
...
There is also another way of expressing $\hat{\beta}_1$ that incorporates partialling out interpretation of OLS. Called Frisch, Waugh, Lovell Theorem, this approach helps us deal with the unwieldiness of deriving estimators in multiple regression. It consists of a seqence of shorter regressions to compute the OLS estimator. Denote $\hat{r}_{ij}$ as the residual observation $i$ from a regression of $X_j$ on all other regressors. Notice that we are regressing one regressor $X$ on all other $X$s. So for $X_1$ we have $\X_i1=\hat{\alpha}_0 + \hat{\alpha}_1X_{i2} + \dots + \hat{\alpha}_kX_{ik} + \hat{r}_{i1}$ or $\hat{r}_{i1} = X_{i1} - \hat{X}_{i1}$ where $\hat{X}_{i1}$ is the fitted value. 
