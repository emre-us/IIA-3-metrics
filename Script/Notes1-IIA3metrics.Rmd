...
= \frac{\displaystyle\sum_{i=1}^nX_i(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} 
= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})Y_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}.
\]

The last two equalities are due to the following algebraic property:
\[
\begin{align}
\sum (X_i-\bar{X})(Y_i-\bar{Y}) 
  &= \sum(Y_iX_i - X_i\bar{Y} - \bar{X}Y_i + \bar{X}\bar{Y}) \\
  &= \sum Y_iX_i - \sum X_i\bar{Y} - \sum\bar{X}Y_i + \sum\bar{X}\bar{Y} \\
  &= \sum Y_iX_i - \bar{Y}\sum X_i - \bar{X}\sum Y_i + n\bar{X}\bar{Y} \\
  &= \sum Y_iX_i - n\bar{Y}\bar{X} - n\bar{X}\bar{Y} + n\bar{X}\bar{Y} \\
  &= \sum Y_iX_i - n\bar{Y}\bar{X} \\
  &= \sum Y_iX_i - \bar{Y}\sum X_i \\
  &= \sum (Y_iX_i - \bar{Y}X_i) \\
  &= \sum (Y_i - \bar{Y})X_i.
\end{align}
\]
We can also get the same result with:
\[
\begin{align}
\sum (X_i-\bar{X})Y_i 
...
In other words, when we have $\sum(X_i - \bar{X}){Y_i-\bar{Y}}$ type of equation, we can always drop one of the means and still obtain the same result.
...
Thus, we can also express $\hat{\beta}_1$ as
\[
\hat{\beta}_1 
= \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})Y_i}{\displaystyle\sum_{i=1}^nX_i^2 - n\bar{X}}
= \frac{\displaystyle\sum_{i=1}^nX_i(Y_i-\bar{Y})}{\displaystyle\sum_{i=1}^nX_i^2 - n\bar{X}}
\]
...
>#### *Decomposition of OLS Estimators* {#olsdecomposition .unnumbered}
>
...
There is also another way of expressing $\hat{\beta}_1$ that incorporates partialling out interpretation of OLS. Called Frisch, Waugh, Lovell Theorem, this approach helps us deal with the unwieldiness of deriving estimators in multiple regression. It consists of a seqence of shorter regressions to compute the OLS estimator. 

First, denote $\hat{r}_{ij}$ as the residual observation $i$ from a regression of $X_j$ on all other regressors. Notice that we are regressing one regressor $X$ on all other $X$s. So for $X_1$ we have $X_{i1}=\hat{\alpha}_0 + \hat{\alpha}_1X_{i2} + \dots + \hat{\alpha}_kX_{ik} + \hat{r}_{i1}$, or, $\hat{r}_{i1} = X_{i1} - \hat{X}_{i1}$ where $\hat{X}_{i1}$ is the fitted value. 

The second step is to regress $Y_i$ against this residual: $Y_i = \gamma_0 + \gamma_1\hat{r}_{i1}+{\epsilon}_i.$ Then $\hat{\gamma}_1$ in this equation is obtained by:
\[
\begin{align}
\hat{\gamma}_1 
  &= \frac{\displaystyle\sum_{i=1}^n(Y_i - \bar{Y})(\hat{r}_{i1} - \bar{\hat{r}}_{i1})}{\displaystyle\sum_{i=1}^n(\hat{r}_{i1} - \bar{\hat{r}}_{i1})^2} \\[6pt]
  &= \frac{\displaystyle\sum_{i=1}^nY_i(\hat{r}_{i1} - \bar{\hat{r}}_{i1})}{\displaystyle\sum_{i=1}^n(\hat{r}_{i1} - \bar{\hat{r}}_{i1})^2} \\[6pt]
  &= \frac{\displaystyle\sum_{i=1}^nY_i\hat{r}_{i1}}{\displaystyle\sum_{i=1}^n\hat{r}_{i1}^2} \ \ \ \ \ \text{ since } \bar{\hat{r}}_{i1} \text{ is residual, } \bar{\hat{r}}_{i1}=0.
\end{align}
\]

What Frisch, Waugh, Lovell Theorem says is that this expression equals to $\hat{\beta}_1$. That is:
\[
\hat{\beta}_1 = \frac{\displaystyle\sum_{i=1}^nY_i\hat{r}_{i1}}{\displaystyle\sum_{i=1}^n\hat{r}_{i1}^2}
\]
which can be easier to obtain than the FOC approach above.

Hwoever, notice that, $\hat{\gamma}_0 \neq \hat{\beta}_0$. This is because $\hat{\gamma}_0 = \bar{Y}-\hat{\gamma}_1\bar{\hat{r}}_{i1}$ and since $\hat{r}_{i1}$ is residual, its mean is $0$. Therefore, $\hat{\gamma}_0 = \bar{Y}.$

One way to think about this expression for $\hat{\beta}_1$ is that it measures the effect of $X_1$ on $Y$ after $X_2,\dots,X_k$ are \textit{partialled out}, or \textit{netted out}. In other words, $\hat{\beta}_1$ is the covariance between $Y$ and variation in $X_1$ that can't be explained by its relation to other $X$s. This Theorem has many uses in theoretical and applied econometrics, and we will discuss it again in time series regressions. 

Now, just as we did for the single regression, we can also derive the standard errors of the estimators. For $k=2$, we have
\[
\begin{align}
Var(\hat{\beta}_0) 
...
\[
R^2 = 1 - \frac{RSS}{TSS} = \frac{\displaystyle\sum_{i=1}^n\hat{u}_i^2}{\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2} = \frac{\displaystyle\sum_{i=1}^n(Y_i - \hat{Y}_i)^2}{\displaystyle\sum_{i=1}^n(Y_i-\bar{Y})^2}.
\]
Also note that
\[
\begin{align*}
...
Since the discussion in [homoskedasticity assumption](#asmptnskedasticity) states that $Var(u_i|X_i)=\sigma_u^2$, the conditional distribution becomes: $(u_i|X_i) \sim N(0, \sigma_u^2).$ 
...
Here, we are assuming that $u_i$ is distributed independently of $u_j$ for all $i \neq j$ meaning that there should not be any systematic association between its values in any two observations. For example, just because the disturbance term is large and positive in one observation, there should not be a tendency for it to behave in the same or a particular way in the next observation. The values of the disturbance terms should be absolutely independent of one another.

This assumption implies that $\sigma_{u_i,u_j}$, the population covariance between $u_i$ and $u_j$, is zero because 
\[
\sigma_{u_i,u_j} = \mathbb{E}\left[ (u_i - \mu_u)(u_j-\mu_j) \right] = \mathbb{E}(u_iu_j) = \mathbb{E}(u_i)\mathbb{E}(u_j)=0.
\]
So this assumption states that conditional on $X_1, \dots, X_n$, the errors in two different time periods are not correlated: $Corr(u_i,u_j | X_1, \dots, X_n)=0$, for all $i\neq j$. If we treat $X_1, \dots, X_n$ as [nonrandom](#fixedregressor), then we can ignore the conditioning on $X_1, \dots, X_n$ and the assumption becomes $Corr(u_i,u_j) = 0$ for all $i\neq j$. When considering whether this assumption holds, it is common to focus on this unconditioned version of the assumption because of its simple interpretation.
...
Finally, if this assumption is not satisfied, OLS will again give inefficient estimates. Violations of this assumption are rare with cross-sectional data.
...
This section discusses why OLS estimators are BLUE. An estimator is BLUE if it is the most efficient estimator among all estimators that are unbiased and are linear functions. We discuss each component of the acronym next.
...
For example, suppose observations $X_1,\dots,X_n$ are i.i.d., and let $\mu_X$ denote the mean of $X_i$. Note that the mean is the same for all $i=1,\dots,n$ because the observations are i.i.d.. The sample mean, $\bar{X} = \frac{1}{n}(X_1+X_2+\dots+X_n) = \frac{1}{n}\sum_{i=1}^nX_i.$ Then the expectation of the sample mean is:
\[
\mathbb{E}(\bar{X}) = \frac{1}{n}\sum_{i=1}^n\mathbb{E}(X_i) = \mu_X
\]
Since the expectation of the sample mean is the population mean, the estimator $\bar{X}$ is unbiased. 
<p style="margin-left: 40px">
$\hookrightarrow$
Also notice that if $n$ is large, the central limit theorem states that this distribution is approximately normal.
</p>
...
<div style="background-color:rgba(237, 231, 225, 1); text-align:left; vertical-align: middle; padding:0px; margin-top:10px; margin-bottom:20px">
> 
#### *Why variance is biased and why subtract 1 from n for variance bias correction?* {#sampvar .unnumbered}
>
The expected value of $X$ is the mean so if $\theta = \mathbb{E}(g(X))$ where $g(X)$ is a transformation of the random variable $X$, and $\theta$ is the expectation of that transformation, then the analog estimator of $\theta$ is the sample mean of $g(X)$. That is, 
\[
\hat{\theta} = \mathbb{E}(\hat{g(X)}) = \frac{1}{n}\sum_{i=1}^n g(X_i).
\]
The population variance can then be written as 
\[
\sigma^2 = \mathbb{E}\left( \left[ X-\mathbb{E}(X) \right]^2 \right) = \mathbb{E}(X^2) - (\mathbb{E}(X))^2 = h\left( \mathbb{E}[g(X)] \right)
\]
where $h(a,b) = a-b^2$ and $g(X) = (x^2, x).$
>
The plug-in estimator is then
\[
h(\hat{\theta}) = h\left( \mathbb{E}[\hat{g(X)}] \right) = h\left(\frac{1}{n}\sum_{i=1}^n g(X_i) \right).
\]
<p style="margin-left: 40px">
$\hookrightarrow$ 
...
*Idealized estimator:* 
<br>
If we knew the population mean $\mu$ then we'd use the estimator $\tilde{\sigma}^2 = \dfrac{1}{n}\displaystyle\sum_{i=1}^n(X_i-\mu)^2$, which is the sample average of iid variables $(X_i - \mu)^2$.
...
\]
Just as we did in [decomposition of OLS estimators](#olsdecomposition), let's now rewrite the expression for $\hat{\beta}_1$ in terms of the regressors and errors. This will be useful in demonstrating the unbiasedness of the OLS estimators. First note that since $Y_i = \beta_0 + \beta_1X_i + u_i$ for a linear regression with one regressor, we have $(Y_i-\bar{Y}) = \beta_1(X_i - \bar{X}) + u_i - \bar{u}$. We can plug this in to the numerator of the expression for $\hat{\beta}_1:$
\[
\begin{align*}
...
\begin{align}
\hat{\beta}_1 
&= \frac{\beta_1\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2 + \sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \\[6pt]
&= \beta_1 + \frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2}
\end{align}
\]
...
&= \beta_1 + \mathbb{E}\left(\frac{\displaystyle\sum_{i=1}^n(X_i-\bar{X})u_i}{\displaystyle\sum_{i=1}^n(X_i-\bar{X})^2} \Bigg{|} X_1,\dots,X_n\right) \\[6pt]
...
The estimator $\hat{{\beta}}_1$ is conditionally unbiased if $\mathbb{E}(\hat{\beta}_1 | X_1,\dots,X_n) = \beta_1$ as discussed [above](#ue). So we just need to show that it is linear. For this, we again start with the expression for $\hat{\beta}_1$ we obtained in the [OLS estimator in single regression](#ols-single) discussion and reproduced in the [unbiased estimators](#ue) discussion above:
\[
\begin{align*}
\hat{\beta}_1 
...
Among unbiased estimators, we prefer the estimator with the smallest variance. For example, [above](#ue) we said both estimators $\bar{X}$ and $X_1$ are unbiased. For a random sample from a population with mean $\mu$ and variance $\sigma^2$, we know that $Var(\bar{X})=\displaystyle\frac{\sigma^2}{n}$. Similarly, $Var(X_1)=\sigma^2$ because $X_1$ is a random draw from the population. Therefore, the difference between $Var(\bar{X})$ and $Var(X_1)$ can be large even for small sample sizes. If, say, $n=10$, then $Var(X_1) = \sigma^2$ is $10$ times as large as $Var(\bar{X}) = \displaystyle\frac{\sigma^2}{10}.$ Thus, $\bar{X}$ is <ins>efficient relative to</ins>  $X_1$ for estimating $\mu$.
...
This does not mean we cannot compare unbiased estimators. One way to do so is to calculate the *mean squared error (MSE)* of the estimators.
...
When an estimator is unbiased, its MSE equals its variance. Since $\bar{X}$ is unbiased, $MSE(\bar{X}) = Var(\bar{X}) = \displaystyle\frac{\sigma^2}{n}.$
...
\[
\begin{align*}
\mathbb{E}(\tilde{\beta}_1 | X_1,\dots,X_n) 
&= \mathbb{E}\left(\beta_0\sum_{i=1}^nw_i + \beta_1\sum_{i=1}^nw_iX_i + \sum_{i=1}^nw_iu_i | X_1,\dots,X_n\right) \\[4pt]
&= \beta_0\sum_{i=1}^nw_i + \beta_1\sum_{i=1}^nw_iX_i
\end{align*}
\]
The last equality is due to the [exogeneity](#assmptnexogen) assumption of Gauss-Markov Theorem whereby

\[
\mathbb{E}\left(\sum_{i=1}^nw_iu_i | X_1,\dots,X_n \right) = \sum_{i=1}^nw_i\mathbb{E}(u_i | X_1,\dots,X_n)=0.
\]
>
