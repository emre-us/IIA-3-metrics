...
Since neither of them exceed the critical value we cannot reject the null hypothesis.
\end{description}
...
Here, however, the second step can be a bit tricky. The restricted model is $y_t = \alpha + 1x_{t1} + \varepsilon.$ Now, to impose restriction that the coefficient on $x_{t1}$ is unity, we need to estimate the following model:
\[
