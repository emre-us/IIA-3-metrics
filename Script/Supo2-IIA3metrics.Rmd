...
\textbf{The variance-covariance matrix of the least squares estimates of } $\beta_1$ and $\beta_2$ is estimated to be:
\[
\begin{bmatrix}
0.7820 & -0.4235 \\
-0.4235 & 0.2549
...
Since there are $n-2$ degrees of freedom and $n=38$ the critical values are $t_{0.025,36} = -2.028$ and $t_{0.975,36}=2.028$.
...
\[
F = \frac{\dfrac{(R_U^2-R_R^2)}{df}}{\dfrac{1-R_U^2}{df}} = \frac{\dfrac{(0.5910 - 0)}{2}}{\dfrac{1-0.5910^2}{36}} = 16.348
\]
\end{description}
...
\item[(E)] \textbf{says that adding lagged values of $q$ to the equation will reduce multicollinearity since it is know that it has a significant effect on food consumption.}
\end{itemize}
...
\begin{description}
\item[Answer:] Multicollinearity, or imperfect multicollinearity, means that two or more of the regressors are highly correlated. This means, there is a linear function of the regressors that is highly correlated with another regressor. To see this, consider the sampling variance of OLS slope estimator:
\[
