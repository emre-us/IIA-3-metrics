...
\]
where $a_i$ are weights assigned by the estimator to each observation $(Y_i,X_i)$ for $i=1,\dots,n$. As usual, we will need to find the expected value and the variance of $\beta_1^*$:

\begin{align*}
\mathbb{E}(\beta_1^*) 
  &= \mathbb{E}\left( \frac{\sum_{i=1}^na_iY_i}{\sum_{i=1}^na_iX_i} \right) \\[6pt]
  &= \mathbb{E}\left( \frac{\sum_{i=1}^na_i(\beta_0 + \beta_1X_i+U_i)}{\sum_{i=1}^na_iX_i} \right) \\[6pt]
  &= \mathbb{E}\left( \frac{\beta_0\sum_{i=1}^na_i + \beta_1\sum_{i=1}^na_iX_i + \sum_{i=1}^na_iU_i}{\sum_{i=1}^na_iX_i} \right) \\[6pt]
  &= \mathbb{E}\left( \beta_0 \frac{\sum_{i=1}^na_i}{\sum_{i=1}^na_iX_i} + \beta_1 + \frac{\sum_{i=1}^na_iU_i}{\sum_{i=1}^na_iX_i} \right) \\[6pt]
  &= \beta_0 \mathbb{E}\left( \frac{\sum_{i=1}^na_i}{\sum_{i=1}^na_iX_i} \right) + \beta_1 + 0 \ \ \ \ \ \text{ since } \mathbb{E}(U_i|X)=0 \\[6pt]
\end{align*}
Therefore, $\beta_1^*$ is unbiased insofar as either $\beta_0=0$ or $\mathbb{E}\left( \frac{\sum_{i=1}^na_i}{\sum_{i=1}^na_iX_i} \right)=0$.

Notice that $\tilde{\beta}_1$ is one case of $\beta_1^*$. 

We can derive the general result for the variance of this estimator as well:

\begin{align*}
Var(\beta_1^*)
  &= Var\left( \frac{\sum_{i=1}^na_iY_i}{\sum_{i=1}^na_iX_i} \right)
  = Var\left( \frac{\sum_{i=1}^na_i(\beta_0 + \beta_1X_i+U_i)}{\sum_{i=1}^na_iX_i} \right) \\[6pt]
  &= \frac{1}{\Big(\sum_{i=1}^na_iX_i\Big)^2} Var\left( \beta_0\sum_{i=1}^na_i + \beta_1\sum_{i=1}^na_iX_i + \sum_{i=1}^na_iU_i \right) \\[6pt]
  &= \frac{1}{\Big( \sum_{i=1}^na_iX_i \Big) ^2} \times Var\left( \sum_{i=1}^na_iU_i \right) \ \ \ \ \ \text{ since $X_i$ and $U_i$ are independent} \\[6pt]
  &= \frac{\sigma_U^2\sum_{i=1}^na_i^2}{\Big(\sum_{i=1}^na_iX_i\Big)^2}. \\[6pt]
\end{align*}
In part (b) we derived $Var(\tilde{\beta})$ as 
\[
...
as desired. Since it is way more than 2 we can reject the null hypothesis.
...
\begin{description}
\item[Answer:] $F$ is defined in equation \ref{eq:2} as:
\[
F = \frac{\dfrac{(RSS_R - RSS_U)}{df}}{\dfrac{RSS_U}{df}}.
\]
...
\begin{align*}
F 
  &= \frac{\dfrac{\bigg((1-R_R^2)TSS_R - (1-R_U^2)TSS_U\bigg)}{df}}{\dfrac{(1-R_U^2)TSS_U}{df}} \\[4pt]
  &= \frac{\dfrac{\dfrac{1}{TSS}\bigg((1-R_R^2)TSS - (1-R_U^2)TSS\bigg)}{df}}{\dfrac{1}{TSS}\dfrac{(1-R_U^2)TSS}{df}} \ \ \ \text{since } TSS_R=TSS_U \\[4pt]
  &= \frac{\dfrac{1-R_R^2 - 1 + R_U^2}{df}}{\dfrac{1-R_U^2}{df}} \\[4pt]
  &= \frac{\dfrac{(R_U^2-R_R^2)}{df}}{\dfrac{1-R_U^2}{df}}
\end{align*}

as desired. Alternatively, we can use $R^2 = \frac{ESS}{TSS}$ or $ESS = R^2\times TSS$ and $RSS = TSS-ESS$ identities:

\begin{align*}
F 
  &= \frac{\dfrac{(RSS_R - RSS_U)}{df}}{\dfrac{RSS_U}{df}} 
  = \frac{\dfrac{(TSS_R - ESS_R) - (TSS_U - ESS_U)}{df}}{\dfrac{TSS_U - ESS_U}{df}} \\[4pt]
  &= \frac{\dfrac{(TSS_R - R_R^2\times TSS_R) - (TSS_U - R_U^2\times TSS_U)}{df}}{\dfrac{TSS_U - R_U^2\times TSS_U}{df}} \\[4pt]
  &= \frac{\dfrac{\Big(TSS_R(1-R_R^2)\Big) - \Big(TSS_U(1-R_U^2)\Big)}{df}}{\dfrac{TSS_U(1-R_U^2)}{df}} \\[4pt]
  &= \frac{\dfrac{TSS(R_U^2 - R_R^2)}{df}}{\dfrac{TSS_U(1-R_U^2)}{df}} \ \ \ \text{ since } TSS_R=TSS_U \\[4pt] 
  &= \frac{\dfrac{(R_U^2 - R_R^2)}{df}}{\dfrac{1-R_U^2}{df}}
\end{align*}
...
We can also arrive to the same conclusion by looking at the restricted model:
\[
\hat{y}_t = \alpha + \varepsilon_t
\]
where $\alpha = \bar{y}$. We can now use this within the expression for $R^2$:

\begin{align*}
R^2 
&= \frac{ESS}{TSS} \\[4pt]
&= \frac{\displaystyle\sum_{t=1}^n(\hat{y}-\bar{y})^2}{TSS} \\[4pt]
&= \frac{\displaystyle\sum_{t=1}^n(\bar{y}-\bar{y})^2}{TSS} \\[4pt]
&= \frac{0}{TSS} \\[4pt]
&= 0.
\end{align*}
...
As well as a large $R_j^2$, a small sample size can lead to a small value of $TSS_j$ and therefore can lead to large sampling variances as well. Therefore large $R_j^2$ and small sample size generate much the same problem in this context: large OLS estimator variance. This in turn means large standard error, which can cause the t-statistic for the coefficient to become not significant. 
