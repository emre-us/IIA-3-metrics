...
```{r results = 'hide'}
property_autocorr_df <- read_excel("../Data/sup4.xls", sheet = "autocorrelation")
str(property_autocorr_df)
```
...
property_autocorr_df <- property_autocorr_df %>% mutate_if(is.character, as.numeric)

# remove N/As from `gprice` and `gwage` only
property_autocorr_df <- property_autocorr_df[!with(property_autocorr_df, 
...
```{r}
SQ3c_lm <- lm(gprice ~ gwage, data = property_autocorr_df)
summary(SQ3c_lm)
```
...
\item The error term $u_t$ is assumed to be normally distributed;
...
\item It contains zones of indecision where presence or absence of aotocorrelation cannot be conclusively determined
...
Here we get a very similar $d$ value of $0.8108223$ to $0.8032315$ obtained in part (i), and would conclude the same way as we did in part (i) - reject the null hypothesis of no postiive autocorrelation.
...
\[
u_t = \rho_1u_{t-1} + \rho_2u_{t-2} + \dots + \rho_pu_{t-p} + \varepsilon_t
\]
...
```{r}
bgtest(SQ3c_lm, order=1, data = property_autocorr_df)
```
...
\]
\item Durbin's alternative test is obtained by performing a Wald test that on the null hypothesis that $\mathbb{H}_0:\eta_1 = \dots = \eta_p = 0$ 
\end{enumerate}
...
Given that $X$s are not exogeneous, BG test is more appropriate here. However, it $F$-tests all slopes including all $X$s which not really that interesting. So it may be preferable to use Durbin's alternative test instead, which is almost identical to the BG test but it does not test whether all the slope coefficients are zero, only whether the coefficients of the lagged residuals are zero.
...
One of the points that was made in the box above was that if we knew $\rho$, we could estimate $\beta_0*$ and $\beta_1$ by regressing $Y_t^*$ on $X_t^*$. Cochrane-Orcutt method is a way to estimate $\rho$.
...
```{r eval = FALSE}
# transform the data
property_autocorr_df <- property_autocorr_df %>%
...
property_autocorr_df$gprice_star[property_autocorr_df$t == 2] <- 
  sqrt((1- rho^2))*property_autocorr_df$gprice[property_autocorr_df$t == 2]

property_autocorr_df$gwage_star[property_autocorr_df$t == 2] <- 
  sqrt((1- rho^2))*property_autocorr_df$gwage[property_autocorr_df$t == 2]

# run the regression on the transformed variables
SQ3e_lm <- lm(gprice_star ~ gwage_star + beta0_star, data = property_autocorr_df)
summary(SQ3e_lm)

# or alternatively use `prais_winsten()` formula from `prais` package
library(prais)
prais_winsten(gprice ~ gwage, data = property_autocorr_df, index = "t", twostep = TRUE)
```
...
/* transform the variables */
  ...
  generate beta0_star = 1 - 0.5945888
  
/* adjust the first observation */
  replace gprice_star=((1 - 0.5945888^2)^(1/2))*gprice if t==2
  replace gwage_star=((1 - 0.5945888^2)^(1/2))*gwage if t==2
  replace beta0_star=((1 - 0.5945888^2)^(1/2)) if t==2
